...
int compute(int **life, int **temp, int N, int *end_count) {
...
int main(int argc, char **argv) {
  ...
  int i, j, k, flag=1, start_count=0, end_count=0 ;
  double t1, t2;
  ...
  for (k = 0; k < NTIMES && flag != 0; k++) {
    flag = 0;
    end_count = 0;
    flag = compute(life, temp, N, &end_count);
  ...
  printf("No. of cells alive at start = %d\n", start_count) ;
  printf("No. of cells alive after %d iterations = %d\n",k,end_count) ;
